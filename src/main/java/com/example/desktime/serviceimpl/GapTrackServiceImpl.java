...
//                long offlineDurationMinutes = ChronoUnit.MINUTES.between(gapStartTime, gapEndTime);
    ...
    public GapUserResponse getUserActivity(String userEmail, LocalDate date) {
        ...
        if (!gapTracks.isEmpty()) {
            LocalDateTime lastGapStartTime = gapTracks.get(gapTracks.size() - 1).getGapStartTime();
            response.setLastActiveTime(lastGapStartTime.format(formatter));
        }
    ...
    public String getProductiveUserActivity(String userEmail, LocalDate date) {
        User user = userRepository.findUserByEmail(userEmail);
        if (user == null) {
            throw new UserNotFoundException();
        }
